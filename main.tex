...
\subsection{さまざまな代数構造の圏論的定義}

Heyting 代数は，
半順序，交わりと結び，最大元と最小元，相対擬補元をそなえた代数構造であり，
ブール代数は相対擬補元が補元に一致する Heyting 代数である．
普通これらの構造は集合論の土台の上で定義されるが，
圏論の概念を用いて定義することもできる．
例えば，交わりと結びは積と余積として，最大元と最小限は終対象と始対象として,
...
\begin{definition}[Heyting 代数]
  有界束 $\cat{C}$ が任意の対象の組について冪をもつとき,
  $\cat{C}$ は有界束 (bounded lattice) であるという．
\end{definition}

\begin{definition}[ブール代数]
  Heyting 代数 $\cat{C}$ について
  \begin{equation}
    \forall x\in\obj{\cat{C}};\ \neg\neg x=x
  \end{equation}
  が成り立つとき $\cat{C}$ は ブール代数であるという．
\end{definition}

\subsection{位相空間論の用語}

\begin{definition}[コンパクト集合]
  位相空間 $(X,\mathcal{O})$ の部分集合 $A\subseteq X$ の任意の開被覆
  $\mathcal{C}\subseteq\mathcal{O}$ に対して有限部分被覆
  $\mathcal{C}_0\in\mathcal{C}$ が存在するとき，
...
\begin{definition}[完全不連結]
  連結成分がすべて一点集合である位相空間を完全不連結 (disconnected)
  であるという．
\end{definition}

\begin{definition}[ハウスドルフ空間]
  連結成分が一点集合でない位相空間をハウスドルフ空間 (Hausdorff space)
  であるという．
\end{definition}

\begin{definition}[ストーン空間]
...
\begin{definition}[ブール代数の圏]
  ブール代数とブール準同型のなす圏を $\blat$ と表記する．
\end{definition}

\begin{definition}[位相空間の圏]
  位相空間と連続写像のなす圏を $\Top$ と表記する．
\end{definition}

\begin{definition}[ストーン空間の圏]
  ストーン空間のなす $\Top$ の充満部分圏を $\stone$ と表記する．
\end{definition}
