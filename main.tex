...
\begin{definition}[圏]
  ...
  \end{itemize}
  また，圏 $\C$ の対象の集まりを $\objs{\C}$，
  射 $\a\to\b$ の集まりを $\mrps{\C}{\a}{\b}$ と表記する．
  \begin{center}
    \begin{tikzpicture}
      \node (A) {$\a$};
      \node [right=1cm of A] (B) {$\b$};
      \node [right=1cm of B] (C) {$\c$};
      \node [above=1.5cm of B] (catc) {$\C$};
...
\begin{definition}[同型]
  \def\a{\obj{a}}%
  \def\b{\obj{b}}%
  圏 $\C$ の対象 $\a,\b$ について同型射 $\a\to\b$ が存在するとき，
...
\begin{definition}[自然変換]
  \def\x{\obj{x}}%
  \def\y{\obj{y}}%
  2つの函手 $F:\C\to\D,G:\C\to\D$ について，
  $\C$ の対象 $\x$ を $\D$ の射 $t_{\x}:F(\x)\to G(\x)$ に対応づける
  写像 $t$ が
  \begin{equation}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node [right=4.0cm of catc] (catd) {$\D$};
...
\begin{definition}[終対象]
  圏 $\C$ および対象 $\obj{t}\in\objs{\C}$ が
  \begin{equation}
    \forall\obj{x}\in\objs{\C};\ |\mrps{\C}{\obj{x}}{\obj{t}}|=1
  \end{equation}
  を満たすとき，
  $\obj{t}$ を $\C$ の終対象 (terminal object) という．
\end{definition}

\begin{definition}[始対象]
  圏 $\C$ および対象 $\obj{i}\in\objs{\C}$ が
  \begin{equation}
    \forall\obj{x}\in\objs{\C};\ |\mrps{\C}{\obj{i}}{\obj{x}}|=1
  \end{equation}
  を満たすとき，
  $\obj{i}$ を $\C$ の始対象 (initial object) という．
\end{definition}
...
\begin{center}
  ...
  \begin{minipage}{0.60\textwidth}
    \centering
    \captionof{table}{終対象と始対象の例\cite{bradley:limits}}
    \begin{tabular}{ccc}
      \toprule
                 & \textbf{終対象} & \textbf{始対象} \\
      \midrule
...
\begin{definition}[定函手]
  圏 $\C,\D$ および対象 $\obj{d}\in\objs{\D}$ について，
  $\C$ の任意の対象を $\obj{d}$ に写し，
  $\C$ の任意の射を $\id{\obj{d}}$ に写す函手を
  $\obj{d}$ の定函手 (constant functor) という．
\end{definition}

\begin{definition}[対角函手]
  圏 $\C,\D$ について，
  任意の対象 $\obj{d}\in\objs{\D}$ を $\obj{d}$ の定函手に写す函手
  $\mrp{\Delta}{\D}{\D^{\C}}$ を対角函手 (diagonal functor) という．
  ただし $\Delta$ は射 $f\in\mrps{\D}{a}{b}$ を，
  任意のコンポーネントが $f$ であるような自然変換
  $\mrp{\Delta(f)}{\Delta(a)}{\Delta(b)}$ に写す．
\end{definition}

\begin{definition}[錐]
  圏 $\C,\D$ および函手 $\mrp{F}{\C}{\D}$ について，
  対象 $\obj{d}\in\objs{\D}$ および自然変換 $\mrp{\psi}{\id{\obj{d}}}{F}$
  の組 $(\obj{d},\psi)$ を $F$ への錐 (cone) という．
\end{definition}

\begin{definition}[余錐]
  圏 $\C,\D$ および函手 $\mrp{F}{\C}{\D}$ について，
  対象 $\obj{d}\in\objs{\D}$ および自然変換 $\mrp{\phi}{F}{\id{\obj{d}}}$
...
\begin{theorem}[極限としての終対象]
  任意の圏 $\C$ および空圏 $\cat{J}$ について，
  自明な図式 $\cat{J}\to\C$ の極限は
  $\C$ の終対象に同型である．
\end{theorem}
\begin{theorem}[余極限としての始対象]
  任意の圏 $\C$ および空圏 $\cat{J}$ について，
  自明な図式 $\cat{J}\to\C$ の余極限は
  $\C$ の始対象に同型である．
\end{theorem}
終対象や始対象の具体的な例をみてみよう．
...
\begin{definition}[積]
  離散圏 $\cat{J}=\{1,2\}$ について，
  函手 $F:\cat{J}\to\C$ の極限を積 (product) といい，
...
\begin{definition}[余積]
  離散圏 $\cat{J}=\{1,2\}$ について，
  函手 $F:\cat{J}\to\C$ の余極限を余積 (coproduct) といい，
...
\begin{definition}[等化子]
  ...
  函手 $F:\cat{J}\to\C$ の極限を等化子 (equalizer) という．
  \begin{center}
    \begin{tikzpicture}
      \node (X) {$X$};
...
\begin{definition}[余等化子]
  圏 $\cat{J}=\{\cdot\rightrightarrows\cdot\}$ について，
  函手 $F:\cat{J}\to\C$ の余極限を余等化子 (coequalizer) という．
  \begin{center}
    \begin{tikzpicture}
      \node (X) {$X$};
      \node [right=1.0cm of X] (Y) {$Y$};
...
\begin{definition}[有限完備]
  圏 $\C$ について任意の有限圏 $\cat{J}$ および函手 $F:\cat{J}\to\C$
  が極限をもつとき，$\C$ は有限完備 (finitely complete) であるという．
\end{definition}

\begin{definition}[完備]
  圏 $\C$ について任意の小圏 $\cat{J}$ および函手 $F:\cat{J}\to\C$
  が極限をもつとき，$\C$ は完備 (complete) であるという．
\end{definition}

\begin{definition}[余完備]
  圏 $\C$ について任意の小圏 $\cat{J}$ および函手 $F:\cat{J}\to\C$
  が余極限をもつとき，$\C$ は余完備 (cocomplete) であるという．
\end{definition}

\begin{theorem}[完備性の必要条件]
  圏 $\C$ が積と等化子をもつならば $\C$ は完備である．
\end{theorem}
...
\def\J{\cat{J}}%
\def\diagram{D}%
\begin{definition}[極限の保存\cite{bradley:limits}]
  函手 $\mrp{\f}{\C}{\D}$ が任意の図式 $\mrp{\diagram}{\J}{\C}$ について
  \begin{equation}
    \f(\lim(\diagram))\simeq\lim(\f\circ\diagram)
  \end{equation}
  を満たすとき，$\f$ は極限を保存する (preserves limits) ，
  または連続 (continuous) であるという．
\end{definition}
\begin{definition}[余極限の保存\cite{bradley:limits}]
  函手 $\mrp{\f}{\C}{\D}$ が任意の図式 $\mrp{\diagram}{\J}{\C}$ について
  \begin{equation}
    \f(\colim(\diagram))\simeq\colim(\f\circ\diagram)
  \end{equation}
  を満たすとき，$\f$ は余極限を保存する (preserves colimits) ，
  または余連続 (cocontinuous) であるという．
\end{definition}
...
\begin{definition}[米田埋め込み]
  ...
  $\mrp{\yoneda}{\C}{\Psh{\C}}$ を米田埋め込み (Yoneda embedding) という．
  \begin{center}
    \begin{tikzpicture}
      % Set
      \node (s) {$\set$};
      \node [below right=0.1cm and -1.5cm of s] (scom) {
        \begin{tikzcd}[row sep=1.0cm, column sep=1.5cm]
          \mrps{\C}{x}{a}\arrow{d}{\psi\circ-}[swap]{\mrps{\C}{x}{\psi}}
          & \mrps{\C}{y}{a}\arrow{l}{-\circ\phi}[swap]{\mrps{\C}{\phi}{a}}
          \arrow{d}{\mrps{\C}{y}{\psi}}[swap]{\psi\circ-} \\
          \mrps{\C}{x}{b}
          & \mrps{\C}{y}{b}\arrow{l}{\mrps{\C}{\phi}{b}}[swap]{-\circ\phi}
        \end{tikzcd}
      };
...
\begin{definition}[余米田埋め込み]
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node [below right=0.1cm and -1.5cm of s] (scom) {
        \begin{tikzcd}[row sep=1.0cm, column sep=1.5cm]
          \mrps{\C}{a}{x}\arrow{r}{\mrps{\C}{a}{\psi}}[swap]{-\circ\phi}
          & \mrps{\C}{a}{y} \\
          \mrps{\C}{b}{x}\arrow{r}{\phi\circ-}[swap]{\mrps{\C}{b}{\phi}}\arrow{u}{\mrps{\C}{\psi}{x}}[swap]{\psi\circ-}
          & \mrps{\C}{b}{y}\arrow{u}{-\circ\psi}[swap]{\mrps{\C}{\psi}{y}}
        \end{tikzcd}
      };
      \node [above left = 0.1cm and  0.5cm of s]    (srec1) {};
      \node [below right=-0.1cm and -0.2cm of scom] (srec2) {};
...
\begin{theorem}[米田埋め込みは極限を保存する]
  任意の局所小圏 $\C$ について，
  米田埋め込み $\mrp{\yoneda}{\C}{\Psh{\C}}$ は極限を保存する．
\end{theorem}
...
\begin{proof}
  ...
  \begin{align*}
    y(x(\a)) & ={x(\a)}_{\c}(\id{\c}) \\
             ...
             & =\a.
  \end{align*}
  $x\circ y=\id{\set^\C(\mrps{\D}{\c}{-},K)}$ を示す．
...
\begin{theorem}[米田埋め込みは充満忠実]
  任意の局所小圏 $\C$ において
  米田埋め込み $\mrp{\yoneda}{\C}{\Psh{\C}}$ は充満忠実
...
2 つの圏 $\C,\D$ を比較して，何らかの意味での「等しさ」を議論したいとする．
最も厳しい「等しさ」は，$\Cat$ において $\C$ と $\D$ が同型 ($\C\simeq\D$)
であること，すなわちある函手の組 $L:\C\rightleftarrows\D:R$ が存在して
$R\circ L=\id{\C},L\circ R=\id{\D}$ が成り立つこととして表現される．
これでは使いづらいというので条件を少し緩めたのが圏同値だ．
これは $R\circ L\simeq\id{\C},L\circ R\simeq\id{\D}$ でさえあればよい．
しかしこれでも窮屈だというのでさらに緩めてみると，
途端に難解で豊かな概念を手にすることができる\cite{bradley:adjoint}．
...
\begin{theorem}[随伴函手の並列定義]\label{thm:adjoint-functor-definitions}
  函手 $F:\C\rightleftarrows\D:G$ について以下の命題は全て同値である:
  \begin{enumerate}
    \item $F\dashv G$．
    \item 自然変換 $\eta:\id{\C}\to GF$ が存在し，
          各 $\eta_c:c\to GF(c)$ が $c$ から $G$ への普遍射である．
    \item 自然変換 $\epsilon:FG\to\id{\D}$ が存在し，
...
\subsection{$\text{始対象}\dashv*\dashv\text{終対象}$}
...
\subsection{$\text{自由}\dashv\text{忘却}$}

群は集合に二項演算という「構造」を付与したものだから，
これを忘れてしまうこともできる．
$\Grp$ の対象 (群) を $\set$ の対象 (集合) に写す函手を忘却函手という．
ときに，この左随伴は何であろうか？

\subsection{$\text{積}\dashv\text{冪}$}
...
\begin{proof}
  ...
  \begin{enumerate}[label=(\roman*)] % chktex 36
    \item 自然性を示す．
          冪函手の定義より，任意の $C,D\in\objs{\C}$ と射 $f:C\to D$ について
          \begin{align*}
            f\circ\epsilon_C
          ...
          よって自然性は示された．
    \item 普遍性を示す．
          冪函手の定義より，任意の $X\in\objs{\C}$ と $g:X\times A\to C$
...

{
  ...
  \begin{definition}[$\monad$-代数\cite{nlab:algebra_over_a_monad}]
    圏 $\cat{C}$ におけるモナド $(\monad,\eta,\mu)$ について，
    対象 $\A\in\objs{\cat{C}}$ と射 $\mrp{\a}{\monad(\A)}{\A}$ が
    以下の図式を可換にするとき，
    $(\A,\a)$ を $\monad$-代数 ($\monad$-algebra) または
    $\monad$ 上の代数 (algebra over $\monad$) という．
    \begin{center}
      \begin{tikzcd}
        \A\arrow{r}{\eta_{\A}}\arrow[equal]{rd}&\monad(\A)\arrow{d}{\a}
        &\monad\monad(\A)\arrow{r}{\monad(\A)}\arrow{d}[swap]{\mu_{\A}}
        &\monad(\A)\arrow{d}{\a} \\
        &\A
        &\monad(\A)\arrow{r}[swap]{\a}&\A
      \end{tikzcd}
      \begin{tikzcd}
      \end{tikzcd}
    \end{center}
  ...
  \begin{definition}[$\monad$-代数準同型\cite{nlab:algebra_over_a_monad}]
    \def\B{B}%
    \def\b{b}%
    \def\f{f}%
    2つの $\monad$-代数 $(\A,\a)$, $(\B,\b)$ について，
    射 $\mrp{\f}{A}{B}$ が右の図式を可換にするとき，
    $\f$ を $\monad$-代数準同型 ($\monad$-algebra homomorphism) という．
    \begin{center}
      \begin{tikzcd}
        \monad(\A)\arrow{r}{\a}\arrow{d}[swap]{\monad(\f)}
        & \A \arrow{d}{\f} \\
        \monad(\B)\arrow{r}[swap]{\b}
        & \B
      \end{tikzcd}
    \end{center}
  \end{definition}
...
\begin{definition}[米田拡張\cite{nlab:yoneda_extension}]
  \def\f{F}%
  函手 $\mrp{\f}{\C}{\D}$ および米田埋め込み
  $\mrp{\yoneda_{\C}}{\C}{\Psh{\C}}$ について，
  左カン拡張 $\lan_{\yoneda_{\C}}\f$ が存在するとき，
  これを米田拡張 (Yoneda extension) という．
  \begin{center}
    \begin{tikzpicture}
      \def\A{\Psh{\C}}%
      \def\K{\yoneda_{\C}}%
      ...
      \draw [->] (M) to node [below]        {$\f$}      (A);
      \draw [->] (C) to node [right=0.1cm]  {$\lan_{\K}\f$} (A);
      \node [above right=0.6cm of C] (astart) {};
      \node [below right=2.1cm and 2.2cm of astart] (aend) {};
      \draw [->] (astart) to node [above right] {$\g$} (aend);
      \node [below right=0.65cm and 0.2cm of C] (eend) {};
      \node [below=1.2cm of eend] (estart) {};
      \draw [->,double] (estart) to node [left] {$\mu$} (eend);
      \node [above right=1.1cm and 0.6cm of M] (sstart) {};
      \node [above right=1.2cm of sstart] (send) {};
...
$\C$ の前層の圏 $\Psh{\C}$ は余完備であった．
ここで，米田埋め込み $\yoneda_{\C}$ は $\C$ の自由余完備化 (free cocompletion)
であるといわれる．
この言明は，前層の圏 $\Psh{\C}$ に限らない任意の余完備な圏 $\D$ への函手
$\C\to\D$ のなかで，米田埋め込みが普遍的であることを意味している．

\begin{theorem}[米田埋め込みは自由余完備化である]
  任意の圏 $\C$ ，米田埋め込み $\mrp{\yoneda_{\C}}{\C}{\Psh{\C}}$ ，
  任意の余完備な圏 $\D$ および函手 $\mrp{F}{\C}{\D}$ について，
  以下の図式を可換にする余連続な函手 $\mrp{\hat{F}}{\Psh{\C}}{\D}$
  が一意に存在し，
  \begin{equation}
    \hat{F}\simeq\lan_{\yoneda_{\C}}F.
  \end{equation}
  \begin{center}
    \begin{tikzcd}
      \Psh{\C}\arrow{rd}{\hat{F}} \\
      \C\arrow{u}{\yoneda_{\C}}\arrow{r}[swap]{F} & \D
    \end{tikzcd}
  \end{center}
\end{theorem}

\subsection{米田埋め込みのカン拡張}
...
\begin{definition}[半順序集合]
  圏 $\C$ について
  \begin{equation}
    \forall x,y\in\objs{\C};|\mrps{\C}{x}{y}|\le1
  \end{equation}
  が成り立つとき，
  $\C$ は半順序集合 (partially ordered set, poset)
  であるという．
\end{definition}
...
\begin{definition}[有界束]
  束 $\C$ が終対象と始対象をもつとき，
  $\C$ は有界束 (bounded lattice) であるという．
\end{definition}

\begin{definition}[ハイティング代数]\label{def:heyting-algebra}%
  有界束 $\C$ が任意の対象の組について冪をもつとき,
  $\C$ はハイティング代数 (Heyting algebra) であるという．
\end{definition}
...
\begin{definition}[ブール代数]\label{def:boolean-algebra}
  ...
  \end{equation}
  が成り立つとき，
  $\C$ は ブール代数 (Boolean algebra) であるという．
\end{definition}
