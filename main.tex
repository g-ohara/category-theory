...
\begin{definition}[共変 hom 函手]
  ...
  \def\x{\obj{x}}%
  \def\y{\obj{y}}%
  局所小圏 $\C$ と対象 $\c\in\objs{\C}$ に対して，
  以下のように定義される函手 $\mrp{\mrps{\C}{\c}{-}}{\C}{\set}$ を
  $\c$ に関する共変 hom 函手 (covariant hom-functor) という．
  \begin{itemize}
    \item 対象 $\x$ を射 $\c\to\x$ の集合 $\mrps{\C}{\c}{\x}$ に写す．
    \item 射 $\mrp{\f}{\x}{\y}$ を写像
          $\mrps{\C}{\c}{\f}:\mrps{\C}{\c}{\x}\ni\g\mapsto\f\circ\g\in\mrps{\C}{\c}{\y}$
          に写す．
  \end{itemize}
\end{definition}
\begin{definition}[反変 hom 函手]
  ...
  \begin{itemize}
    \item 対象 $\x$ を射 $\x\to\c$ の集合 $\mrps{\C}{\x}{\c}$ に写し，
    \item 射 $\mrp{\f}{\x}{\y}$ を写像
          $\mrps{\C}{\f}{\c}:\mrps{\C}{\y}{\c}\ni\g\mapsto\g\circ\f\in\mrps{\C}{\x}{\c}$
          に写す
  \end{itemize}
  函手 $\mrp{\mrps{\C}{-}{\c}}{\op{\C}}{\set}$ を
...
\begin{definition}[米田埋め込み]
  ...
  \begin{itemize}
    \item 対象 $\a$ を表現可能函手
          $\mrp{\mrps{\C}{-}{\a}}{\op{\C}}{\set}$ に写し，
    \item 射 $\mrp{\f}{\c}{\d}$ を自然変換
          $\mrp{\mrps{\C}{-}{\f}}{\mrps{\C}{-}{\c}}{\mrps{\C}{-}{\d}}$ に写す
  \end{itemize}
  函手 $\mrp{\Y}{\C}{\set^{\op{\C}}}$
  を米田埋め込み (Yoneda embedding) という．
  ただし，任意の対象 $x\in\objs{\C}$ について，
  自然変換 $\Y(\f)=\mrps{\C}{-}{\f}$ の $x$ におけるコンポーネントは
  \begin{equation}
    \mrps{\C}{x}{\f}:\mrps{\C}{x}{\c}\ni g\mapsto\phi\circ g\in\mrps{\C}{x}{\d}
  \end{equation}
  である．
  \begin{center}
